...
该目录在 API 边界按引擎隔离。DSH 会话排除 `zcode` provider，但保留 `zai` 等普通 GLM route；ZCode 会话只展示 ZCode provider 的 `glm-5.3-flash` 和 `glm-5.3`。跨引擎选模和发起对话都会被拒绝，不支持的 ZCode 模型 ID 会在进程启动前失败，ZCode 选模也不会覆盖已保存的 DSH 默认模型。
