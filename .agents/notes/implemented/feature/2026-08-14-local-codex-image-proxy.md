...
The local `scripts/codex-image-worker.mjs` polls through the authenticated `bohr sandbox` CLI every ten seconds. It never exposes a public claim endpoint. Claim is an atomic pending-to-claimed rename with a renewable lease; stale claims requeue. While busy, the worker renews both lease and heartbeat, runs `codex exec --json --ephemeral --sandbox workspace-write`, asks `$imagegen` for exactly one image, validates its signature and size, uploads to a private temporary sandbox path, then asks the sandbox to verify length and SHA-256 before atomically publishing the final result. Cancellation markers prevent a late worker result from reviving an aborted call.
...
Image generation works only while a local worker is heartbeating, and each request waits for the polling interval before claim. The local machine retains Codex credentials and generation cost; the remote sandbox retains only prompts, bounded context, status records, and generated images. Multiple identifiers share one worker process but have distinct queue and route namespaces.
