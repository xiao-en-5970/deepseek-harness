...
| `ctx.codexImageProxy` | `core` | [`codex-image-proxy`](../packages/image/codex-image-proxy) | - | [`tool-codex-image-proxy`](../packages/image/tool-codex-image-proxy) | - | The Web host owns the durable request queue and image route; the model-facing tool submits bounded requests and renders worker results. |
