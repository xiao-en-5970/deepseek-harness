...
| `@deepseek-ai/dsh-tool-codex-image-proxy` | `generate_image` | `ctx.tools`、`ctx.systemPrompt`、`ctx.codexImageProxy`、`ctx.fs`、`ctx.attachments` | `tool/call`、`durable image-generation queue request`、`tool/result` | - | Web 宿主只拥有一份持久队列和同源图片路由；每 Agent 实例只注册面向模型的消费端。发布请求前必须存在新鲜的本机 worker 心跳。 |
...
通过用户本机的 Codex 文生图 worker 生成或编辑一张图片。支持文生图、工作区文件和当前对话图片附件，不要求语言模型读取像素。
...
```json
{
  ...
  "properties": {
    "prompt": {
      "type": "string",
      "description": "Complete, self-contained visual generation request."
    },
    "context": {
      "type": "string",
...
来源：[`packages/image/tool-codex-image-proxy/src/index.ts`](../packages/image/tool-codex-image-proxy/src/index.ts)

Web 宿主只拥有一份持久队列和同源图片路由；每 Agent 实例只注册面向模型的消费端。发布请求前必须存在新鲜的本机 worker 心跳。
