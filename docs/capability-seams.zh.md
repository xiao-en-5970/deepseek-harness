...
flowchart LR
  ...
  pkg_codex_image_proxy["codex-image-proxy"]
  svc_codexImageProxy["ctx.codexImageProxy<br/>Local Codex image generation queue"]
  pkg_tool_codex_image_proxy["tool-codex-image-proxy"]
  pkg_llm["llm"]
  ...
  pkg_codex_image_proxy --> svc_codexImageProxy
  ...
  svc_codexImageProxy --> pkg_tool_codex_image_proxy
...
| `ctx.codexImageProxy` | `core` | [`codex-image-proxy`](../packages/image/codex-image-proxy) | - | [`tool-codex-image-proxy`](../packages/image/tool-codex-image-proxy) | - | Web 宿主拥有持久请求队列和图片路由；面向模型的工具提交有界请求并渲染 worker 结果。 |
