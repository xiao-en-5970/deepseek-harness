...
function scriptedApi(overrides: {
  ...
  return {
    ...
    host: {
      ...
      listWorkspaceFiles: r => ok(r, { path: r.payload.path, entries: [], truncated: false }),
      createFile: r => ok(r, { path: `${r.payload.path}/${r.payload.name}` }),
      beginDirectoryUpload: r => ok(r, { uploadId: 'upload-1', path: '/t/upload', maxChunkBytes: 1024 }),
      writeDirectoryUpload: r => ok(r, { offset: r.payload.offset }),
      completeDirectoryUpload: r => ok(r, { path: `/t/${r.payload.uploadId}` }),
      abortDirectoryUpload: r => ok(r, { aborted: true as const }),
      beginFileUpload: r => ok(r, { uploadId: '00000000-0000-4000-8000-000000000002', path: `${r.payload.parentPath}/${r.payload.name}`, maxChunkBytes: 1024 }),
      writeFileUpload: r => ok(r, { offset: r.payload.offset }),
      completeFileUpload: r => ok(r, { path: `/t/${r.payload.uploadId}` }),
      abortFileUpload: r => ok(r, { aborted: true as const }),
