...
The bwrap profile explicitly requests a user namespace. This keeps the same full mount policy usable both for ordinary users and for containerized root, which often has no outer `CAP_SYS_ADMIN` even when the kernel allows user namespaces. The functional probe uses the exact same profile builder as real commands, so selection cannot reject a profile that execution would accept.
