...
bwrap profile 会显式请求用户命名空间。这使同一套完整 mount 策略既能用于普通用户，也能用于容器内 root；后者即使内核允许用户命名空间，也经常没有外层 `CAP_SYS_ADMIN`。功能探针与真实命令复用完全相同的 profile builder，因此不会出现探针拒绝、真实执行却本可接受的分叉。
