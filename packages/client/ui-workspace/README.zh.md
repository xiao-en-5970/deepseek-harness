...
当所选 Session 属于一个已注册 Workspace 时，侧边栏区头会显示文件按钮，用该 Workspace 的持久文件树替换 Session 投影。目录按单层懒加载，可以选作**新建文件**、**新建目录**、多文件**上传文件**或刷新的目标；返回按钮恢复原先的分组／扁平 Session 浏览器。文件按顺序上传，并遵循 Host 公布的分块上限，使每个 base64 JSON 请求都低于配置的代理限制。创建与上传绝不替换已有条目，错误会留在文件视图中供用户重试。该树是基于 Host browse capability 的浏览器界面，不是面向模型的 `ctx.fs` 消费方。设计依据：[Workspace 文件浏览器 Agent Note](../../../.agents/notes/implemented/feature/2026-08-14-workspace-file-explorer.md)。
