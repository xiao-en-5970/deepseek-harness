...
- **No search, no multi-select, and no rename or delete** — the dialog lists, creates, and uploads directories; a target is reached by navigating, editing the path, or filtering the last pane by prefix.
...
- **Empty local directories are omitted** — `webkitdirectory` exposes files but no standalone empty-directory entries. Browser support for this non-standard input attribute is required.
