...
The tenant row capability-probes `GET /__dsh_tenant/current`. It stays absent under an ordinary `dsh web` server and, under `dsh tenant-web`, shows only the current browser identifier (never a tenant roster). **Switch identifier** performs a full navigation to the blocking selector; a named tenant also gets **Return to default**. The row explains that provider keys inherit from the default space until the user stores a tenant-local override in Models. The endpoint returns routing identity only — no paths, credentials, or other identifiers.
...
- The built-in identifier row appears only behind `dsh tenant-web`; ordinary Web deployments keep their feature-contributed General rows only.
