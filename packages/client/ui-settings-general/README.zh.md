...
标识符行通过 `GET /__dsh_tenant/current` 探测能力。普通 `dsh web` 服务下它保持隐藏；在 `dsh tenant-web` 下只显示当前浏览器的标识符，从不列出租户清单。**切换标识符**会整页导航到阻塞式选择器；命名租户还会出现**切回默认空间**。该行会说明：提供方 Key 在用户通过「模型」页保存租户本地覆盖之前继承自默认空间。端点只返回路由标识，不返回路径、凭据或其他标识符。
...
- 内置标识符行只在 `dsh tenant-web` 后出现；普通 Web 部署仍只保留由各功能贡献的「通用」行。
