...
describe('ui-settings-general apply', () => {
  ...
  it('fills all five seats for declarations before or after apply', async () => {
    ...
    expect(before.slots.entries('settings.general.item')[0]).toMatchObject({
      component: TenantIdentityRow,
      options: { id: 'tenant-identity', order: -100 },
      locale: 'settings',
    })
    ...
    await vi.waitFor(() => {
      ...
      expect(after.slots.entries('settings.general.item')[0]!.component).toBe(TenantIdentityRow)
    })
  })
  ...
  it('re-registers after an HMR collapse of the declaring chain (stale disposers must not block)', async () => {
    ...
    expect(b.slots.entries('settings.general.item')[0]!.component).toBe(TenantIdentityRow)
